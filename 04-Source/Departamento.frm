...
Public Sub Excluir()
    ...
    If AbreTabela(Dicionário, "CADASTRO", "DEPARTAMENTO - SEÇÃO", DBCadastro, TBLDepartamentoSeção, TBLTabela, dbOpenTable) Then
        TBLDepartamentoSeção.Index = "DEPARTAMENTOSEÇÃO1"
        TBLDepartamentoSeção.Seek ">=", txtCódigo
        If Not TBLDepartamentoSeção.NoMatch Then
            If TBLDepartamentoSeção("CÓDIGO DO DEPTO") = txtCódigo Then
                MsgBox "Relação violada!" + vbCr + "Para apagar este departamento, antes é necessário apagar" + vbCr + "todos os 'departamentos-seção' dele dependente.", vbExclamation, "Aviso"
    ...
    If Err <> 0 Then
        GeraMensagemDeErro "Departamento - Excluir - " & txtDescrição, True
    ...
    Log gUsuário, "Exclusão - Departamento: " & txtCódigo & " - " & txtDescrição
...
Public Sub PosRecords()
    ...
    TBLDepartamento.Seek "=", txtCódigo
...
Private Sub GetRecords()
    ...
    If Not lAllowConsult Then
        ...
        Exit Sub
    End If
    txtCódigo = TBLDepartamento("CÓDIGO")
    txtCódigoAnterior = txtCódigo
    txtDescrição = TBLDepartamento("DESCRIÇÃO")
...
Private Function SetRecords()
    ...
    If (txtCódigo <> txtCódigoAnterior) And Not lInserir Then
        If AbreTabela(Dicionário, "CADASTRO", "DEPARTAMENTO - SEÇÃO", DBCadastro, TBLDepartamentoSeção, TBLTabela, dbOpenTable) Then
            ...
            If Not TBLDepartamentoSeção.NoMatch Then
                If TBLDepartamentoSeção("CÓDIGO DO DEPTO") = txtCódigoAnterior Then
                    AchouDepartamentoSeção = True
                    Confirmação = MsgBox("Você necessita alterar os 'Departamentos-Seção' relacionados com este departamento !" + vbCr + "Deseja realizar agora as alterações de" + vbCr + "todas os 'departamentos-seção' dele dependente?", vbYesNo + vbQuestion + vbDefaultButton2, "Aviso")
                End If
            Else
    ...
    TBLDepartamento("CÓDIGO") = txtCódigo
    TBLDepartamento("DESCRIÇÃO") = txtDescrição
    If lInserir Then
        TBLDepartamento("USERNAME - CRIA") = gUsuário
    ...
    End If
    If lAlterar Then
        TBLDepartamento("USERNAME - ALTERA") = gUsuário
    ...
    If AchouDepartamentoSeção Then
        SQL = "Update [DEPARTAMENTO - SEÇÃO] Set [CÓDIGO DO DEPTO]= '" + txtCódigo + "' Where [CÓDIGO DO DEPTO]= '" + txtCódigoAnterior + "'"
...
Erro:
    If Err <> 0 Then
        ...
        GeraMensagemDeErro "Departamento - SetRecords" - txtDescrição, True
        SetRecords = False
        Exit Function
    End If
    ...
    If Not lInserir Then
        For Cont = 1 To Forms.Count - 1
            If Forms(Cont).Name = "frmDepartamentoSeção" Then
                If Forms(Cont).txtCódigoDepartamento = txtCódigoAnterior Then
                    Forms(Cont).txtCódigoDepartamento = txtCódigo
                    Forms(Cont).txtDescriçãoDepartamento = txtDescrição
    ...
    If lInserir Then
        Log gUsuário, "Inclusão - Departamento: " & txtCódigo & " - " & txtDescrição
    Else
        Log gUsuário, "Alteração - Departamento: " & txtCódigo & " - " & txtDescrição
    End If
...
Private Sub Form_Load()
    ...
    StatusBarAviso = "Pronto"
    Relatório = AddPath(AplicaçãoPath, "REPORT\DEPARTAMENTO.RPT")
    TotalDatabaseName = 1
